...
function DinoGame() {
  ...
  useEffect(() => {
    ...
    const gameLoop = () => {
      ...
      // Draw logo instead of rectangle
      if (logoRef.current) {
        ctx.save();

        // Calculate center point for rotation
        const centerX = dinoX + dinoSize / 2;
        const centerY = game.dinoY + dinoSize / 2;

        // Move to center for rotation
        ctx.translate(centerX, centerY);

        // Add walking animation rotation
        if (!game.isJumping) {
          game.walkAnimation += 0.1;
          game.rotation = game.walkAnimation; // Full 360 degree rotation
        } else {
          game.rotation = 0; // No rotation when jumping
        }
        ...
        // Fallback to rectangle if logo not loaded
        ctx.fillStyle = "#535353";
    ...
    <div className="fixed inset-0 flex items-center justify-center flex-col gap-3 bg-[#00000050] bg-opacity-50 z-50">
      ...
      <div className="bg-white p-4 rounded-lg shadow-lg">
        ...
        <div className="text-center mt-2 text-sm text-gray-600">
          Space to start the game and jump your BEARNO
        </div>
      </div>
    </div>
  );
